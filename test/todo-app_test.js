...
describe('TodoApp', () => {
  ...
  it('new todo is undone and toogleTodo changes it', async () => {
    const element = await fixture(html`<todo-app></todo-app>`);
    element.tasks = [firstTask, secondTask];

    await element.addTodo('tercera tarea');
    const task = await element.tasks.find(t => t.task === 'tercera tarea')
    task.completed.should.equal(false)

    await element.toogleTodo(task.id)
